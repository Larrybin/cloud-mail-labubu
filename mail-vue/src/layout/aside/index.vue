  ...
  <el-scrollbar class="scroll">
    <div>
      ...
      <el-menu :collapse="false" text-color="#fff" active-text-color="#fff" style="margin-top: 10px">
        ...
        <div class="manage-title" v-perm="['all-email:query','subscriber:query','user:query','role:query','setting:query','analysis:query','reg-key:query']">
          <div>{{$t('manage')}}</div>
        </div>
        ...
        <el-menu-item @click="router.push({name: 'subscriber'})" index="subscriber" v-perm="'subscriber:query'"
                      :class="route.meta.name === 'subscriber' ? 'choose-item' : ''">
          <Icon icon="mdi:account-box-multiple-outline" width="22" height="22" />
          <span class="menu-name" style="margin-left: 20px">{{$t('subscribers')}}</span>
        </el-menu-item>
